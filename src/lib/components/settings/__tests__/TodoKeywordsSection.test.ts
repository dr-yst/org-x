...
describe('TodoKeywordsSection', () => {
    ...
    it('should handle add active keyword', async () => {
        ...
        const activeAddButton = addButtons.find(button =>
            button.querySelector('svg') &&
            button.closest('.flex')?.querySelector('input[placeholder="Add keyword..."]') === activeInput
        );
    ...
    it('should handle add closed keyword', async () => {
        ...
        const addButtons = screen.getAllByRole('button');
        const closedAddButton = addButtons.find(button =>
            button.querySelector('svg') &&
            button.closest('.flex')?.querySelector('input[placeholder="Add keyword..."]') === closedInput
        );
        ...
        if (closedAddButton) {
            ...
            await waitFor(() => {
                expect(mockCommands.addClosedTodoKeyword).toHaveBeenCalledWith('ARCHIVED');
            });
        }
    });
    ...
    it('should allow dismissing error messages', async () => {
        ...
        const dismissButton = screen.getByRole('button', { name: '' }); // X button typically has no text
