<script lang="ts">
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === 'j' || event.key === 'ArrowDown') {
      ...
      if (filteredHeadlines.length > 0) {
        focusedIndex = Math.min(focusedIndex + 1, filteredHeadlines.length - 1);
  ...
  function handleQuickAction(action: string) {
    if (focusedIndex < 0 || focusedIndex >= filteredHeadlines.length) return;
...
<div class="w-full h-full p-4 {showDetailView ? 'grid grid-cols-2 gap-4' : ''}">
        ...
        <div class="flex items-center gap-4 mb-4 text-sm text-gray-600">
          ...
          {#if document.category}
            <div class="flex items-center gap-1">
              ...
              <span>Category: <span class="font-medium">{document.category}</span></span>
            </div>
          {/if}
        </div>
    ...
    <div class={showDetailView ? "" : "w-full"}>
      {#if !showDetailView}
        <div class="mb-4 flex flex-col gap-2">
          <h3 class="text-xl font-semibold text-gray-800">Task List</h3>
          <p class="text-sm text-gray-600">
            Keyboard shortcuts: <kbd class="px-1.5 py-0.5 bg-gray-100 border rounded text-xs">j/↓</kbd> Next item,
            <kbd class="px-1.5 py-0.5 bg-gray-100 border rounded text-xs">k/↑</kbd> Previous item,
            <kbd class="px-1.5 py-0.5 bg-gray-100 border rounded text-xs">f</kbd> Change filter,
            <kbd class="px-1.5 py-0.5 bg-gray-100 border rounded text-xs">.</kbd> Quick actions,
            <kbd class="px-1.5 py-0.5 bg-gray-100 border rounded text-xs">Enter</kbd> View details
          </p>
        </div>
      {/if}
      ...
      <div class="relative">
        {#if !showDetailView}
          ...
          {#if showQuickActions && focusedIndex >= 0 && focusedIndex < filteredHeadlines.length}
            <div class="absolute right-4 top-10 w-64 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
              ...
              <div class="p-1">
                ...
                <button 
                  class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 rounded-md flex items-center gap-2" 
                  onclick={() => handleQuickAction('mark-done')}>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                ...
                <button 
                  class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 rounded-md flex items-center gap-2" 
                  onclick={() => handleQuickAction('priority-up')}>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                ...
                <button 
                  class="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 rounded-md flex items-center gap-2" 
                  onclick={() => handleQuickAction('priority-down')}>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
      ...
      {#if showDetailView}
        <div class="w-full">
          <div class="mb-4 flex justify-between items-center">
            ...
            <button
              class="px-2 py-1 rounded-md text-sm text-gray-600 hover:bg-gray-100 flex items-center gap-1"
              onclick={() => { showDetailView = false; selectedHeadline = null; }}>
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
  ...
  {:else}
    <div class="p-4 border border-yellow-500 bg-yellow-50 text-yellow-700 rounded">
      No document loaded.
    </div>
  {/if}
</div>