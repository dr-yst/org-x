    ...
    async function runTests() {
        ...
        try {
            ...
            if (loadResult.status === "ok") {
                ...
                addTestResult(
                    `✓ Settings loaded: ${settings.monitored_paths.length} paths`,
                );
            } else {
            ...
            const statusResult1 = await commands.checkPathMonitoringStatus(
                "/test/non-existent.org",
            );
            if (statusResult1.status === "ok") {
                addTestResult(
                    `✓ Non-monitored path status: ${statusResult1.data}`,
                );
            } else {
                addTestResult(
                    `✗ Failed to check status: ${statusResult1.error}`,
                );
            }
            ...
            if (addPathResult.status === "ok") {
                ...
                addTestResult(
                    `✓ Path added: ${settings.monitored_paths.length} total paths`,
                );
            } else {
            ...
            const statusResult2 =
                await commands.checkPathMonitoringStatus("/test/example.org");
            if (statusResult2.status === "ok") {
                addTestResult(`✓ Monitored path status: ${statusResult2.data}`);
                if (!statusResult2.data) {
                    addTestResult(
                        "⚠ Warning: Path was added but is not showing as monitored",
                    );
                }
            } else {
                addTestResult(
                    `✗ Failed to check status: ${statusResult2.error}`,
                );
            }

            // Test 5: Disable the path
            addTestResult(
                "Test 5: Disabling parsing for the monitored path...",
            );
            const disableResult = await commands.setPathParseEnabled(
                "/test/example.org",
                false,
            );
            if (disableResult.status === "ok") {
                settings = disableResult.data;
                const path = settings.monitored_paths.find(
                    (p) => p.path === "/test/example.org",
                );
                addTestResult(
                    `✓ Path parsing disabled: parse_enabled=${path?.parse_enabled}`,
                );
            } else {
                addTestResult(
                    `✗ Failed to disable path: ${disableResult.error}`,
                );
            }
            ...
            if (statusResult3.status === "ok") {
                ...
                if (statusResult3.data) {
                    addTestResult(
                        "⚠ Warning: Disabled path is still showing as monitored",
                    );
                }
            } else {
                addTestResult(
                    `✗ Failed to check status: ${statusResult3.error}`,
                );
            }

            // Test 7: Re-enable the path
            addTestResult(
                "Test 7: Re-enabling parsing for the monitored path...",
            );
            const enableResult = await commands.setPathParseEnabled(
                "/test/example.org",
                true,
            );
            if (enableResult.status === "ok") {
                ...
                addTestResult(
                    `✓ Path parsing re-enabled: parse_enabled=${path?.parse_enabled}`,
                );
            } else {
                addTestResult(
                    `✗ Failed to re-enable path: ${enableResult.error}`,
                );
            }
            ...
            if (statusResult4.status === "ok") {
                ...
                addTestResult(
                    `✗ Failed to check final status: ${statusResult4.error}`,
                );
            }
            ...
            const removeResult =
                await commands.removeMonitoredPath("/test/example.org");
            if (removeResult.status === "ok") {
                settings = removeResult.data;
                addTestResult(
                    `✓ Path removed: ${settings.monitored_paths.length} total paths`,
                );
            } else {
            ...
            if (statusResult5.status === "ok") {
                ...
                if (statusResult5.data) {
                    addTestResult(
                        "⚠ Warning: Removed path is still showing as monitored",
                    );
                }
            } else {
                addTestResult(
                    `✗ Failed to check status after removal: ${statusResult5.error}`,
                );
            }
    ...
    async function clearSettings() {
        ...
        try {
            ...
            if (clearResult.status === "ok") {
                ...
                addTestResult(
                    `✗ Failed to clear settings: ${clearResult.error}`,
                );
            }
        } catch (error) {
...
<div class="p-6 max-w-4xl mx-auto">
    <h1 class="text-2xl font-bold mb-4">
        Monitoring Commands Integration Test
    </h1>
    ...
    {#if settings}
        <div class="mb-4 p-4 bg-gray-100 rounded">
            ...
            <div class="text-sm">
                ...
                <div>
                    Parse Enabled Paths: {settings.monitored_paths.filter(
                        (p) => p.parse_enabled,
                    ).length}
                </div>
                        ...
                        <ul class="mt-1 ml-4">
                            {#each settings.monitored_paths as path}
                                <li class="text-xs">
                                    ...
                                    parse_enabled: {path.parse_enabled})
                                </li>
                            {/each}
                        </ul>
                    </details>
    ...
    <div
        class="bg-black text-green-400 p-4 rounded font-mono text-sm h-96 overflow-y-auto"
    >
        {#if testResults.length === 0}
            <div class="text-gray-400">
                Click "Run Tests" to start the monitoring commands integration
                test...
            </div>
        {:else}
